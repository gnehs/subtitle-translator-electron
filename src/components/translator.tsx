  ...
  async function retry(index: number) {
    ...
    const completion: any = await openai.createChatCompletion({
      ...
      messages: [
        {
          role: "system",
          content: `You are a program responsible for translating subtitles. Your task is to output the specified target language based on the input text. Do not create the following subtitles on your own. Do not output any text other than the translation. You will receive the subtitles as array that needs to be translated, as well as the previous translation results and next subtitle. Transliterate the person's name into the local language. Target language: ${targetLanguage}\n\n${additionalNotes}`
        },
        {
          role: "user",
    ...
    for (let i = 0; i < subtitle.length; i++) {
      ...
      const completion: any = await openai.createChatCompletion({
        ...
        messages: [
          {
            role: "system",
            content: `You are a program responsible for translating subtitles. Your task is to output the specified target language based on the input text. Do not create the following subtitles on your own. Do not output any text other than the translation. You will receive the subtitles as array that needs to be translated, as well as the previous translation results and next subtitle. If you need to merge the subtitles with the following line, simply repeat the translation. Transliterate the person's name into the local language. Target language: ${targetLanguage}\n\n${additionalNotes}`
          },
